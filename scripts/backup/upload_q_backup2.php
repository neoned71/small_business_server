...
else
{
	$option_A=new stdClass;
	$option_A->type='text';
	$option_A->value=stripslashes(handle_escaping($dbc,$_POST['option_A']));
	array_push($options,$option_A);
}
...
else
{
	$option_B=new stdClass;
	$option_B->type='text';
	$option_B->value=stripslashes(handle_escaping($dbc,$_POST['option_B']));
	array_push($options,$option_B);
}
...
else
{
	$option_C=new stdClass;
	$option_C->type='text';
	$option_C->value=stripslashes(handle_escaping($dbc,$_POST['option_C']));
	array_push($options,$option_C);
}
...
else
{
	$option_D=new stdClass;
	$option_D->type='text';
	$option_D->value=stripslashes(handle_escaping($dbc,$_POST['option_D']));
	array_push($options,$option_D);
}
...
$id=insert_question($dbc,$subject_id, $topic_id, $difficulty,$question,addslashes(json_encode($options)),$correct_ans,addslashes(json_encode($solution)),$image_name);
